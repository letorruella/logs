...
query = """ create or replace view path_ok as select log.path, substring
            (path from 10 for 19), COUNT(*) AS VIEWS FROM log where status
...
query = """create or replace view total_status as select  to_char
            (time, 'Month DD, YYYY') as date, count(status) as count
...
# Top authors
query = """select name, sum(views)as views from pop_authors group by name
            order by views desc;"""
...
query = """select round((errors/count::float)*100) results, err_status.date
